...
public:
    ...
    bool WriteZVPXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZVPXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZVPXSeed_deprecated(uint256& seed);
    bool EraseZVPXSeed();
    bool EraseZVPXSeed_deprecated();

    bool WriteZVPXCount(const uint32_t& nCount);
    bool ReadZVPXCount(uint32_t& nCount);
