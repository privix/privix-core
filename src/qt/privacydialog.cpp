                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zVPXpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    ...
    // Set labels/buttons depending on SPORK_19 status
    updateSPORK23Status();
}
...
void PrivacyDialog::on_pushButtonMintzVPX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_19_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVPX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zVPX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zVPX in ") +
...
void PrivacyDialog::on_pushButtonSpendzVPX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_19_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVPX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
...
void PrivacyDialog::on_pushButtonZVpxControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZVpxControlDialog* zVpxControl = new ZVpxControlDialog(this);
    zVpxControl->setModel(walletModel);
    zVpxControl->exec();
}

void PrivacyDialog::setZVpxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzVPXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzVPX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Privix Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // Warn for additional fees if amount is not an integer and change as zVPX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " VPX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zVPX</b>";
    ...
    // use mints from zVPX selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZVpxControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZVpxControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zVPX was spent successfully update the addressbook with the label
    ...
    // Clear zvpx selector in case it was used
    ZVpxControlDialog::setSelectedMints.clear();
    ui->labelzVPXSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zVPX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Vpx, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zVPX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Update/enable labels and buttons depending on the current SPORK_19 status
    updateSPORK23Status();
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zVPX </b>";
