...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::VPX);
    ...
    if (!settings.contains("nAnonymizePrivixAmount"))
        settings.setValue("nAnonymizePrivixAmount", 1000);

    nAnonymizePrivixAmount = settings.value("nAnonymizePrivixAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizePrivixAmount"))
        SoftSetArg("-anonymizeprivixamount", settings.value("nAnonymizePrivixAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in privix.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizePrivixAmount:
            return QVariant(nAnonymizePrivixAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizePrivixAmount:
            nAnonymizePrivixAmount = value.toInt();
            settings.setValue("nAnonymizePrivixAmount", nAnonymizePrivixAmount);
            emit anonymizePrivixAmountChanged(nAnonymizePrivixAmount);
            break;
        case CoinControlFeatures:
