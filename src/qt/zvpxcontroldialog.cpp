...
#include "ui_zvpxcontroldialog.h"
...
std::set<std::string> ZVpxControlDialog::setSelectedMints;
std::set<CMintMeta> ZVpxControlDialog::setMints;

ZVpxControlDialog::ZVpxControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZVpxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZVpxControlDialog::~ZVpxControlDialog()
{
    delete ui;
}

void ZVpxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVpxControlDialog::updateList()
{
...
void ZVpxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVpxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZVpx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZVpxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZVpxControlDialog::GetSelectedMints()
{
...
void ZVpxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
