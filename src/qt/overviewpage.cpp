...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::VPX)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount vpxAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance + watchImmatureBalance;    
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // Combined balances
	CAmount availableTotalBalance = vpxAvailableBalance;
	CAmount sumTotalBalance = nTotalBalance;

    // VPX labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, vpxAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    QString automintHelp = tr("Current percentage of zVPX.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in privix.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in privix.conf");
    }
    ...
    bool showWatchOnlyVPXPending = watchUnconfBalance != 0;
    bool showVPXLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyVPXLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;

    ui->labelBalance->setVisible(showVPXAvailable || showWatchOnlyVPXAvailable);
    ui->labelBalanceText->setVisible(showVPXAvailable || showWatchOnlyVPXAvailable);
    ui->labelWatchAvailable->setVisible(showVPXAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showVPXPending || showWatchOnlyVPXPending);
    ui->labelPendingText->setVisible(showVPXPending || showWatchOnlyVPXPending);
    ui->labelWatchPending->setVisible(showVPXPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showVPXLocked || showWatchOnlyVPXLocked);
    ui->labelLockedBalanceText->setVisible(showVPXLocked || showWatchOnlyVPXLocked);
    ui->labelWatchLocked->setVisible(showVPXLocked && showWatchOnly);
    ui->labelImmature->setVisible(showImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelImmatureText->setVisible(showImmature || showWatchOnlyImmature);
    ui->labelWatchImmature->setVisible(showImmature && showWatchOnly);
	bool showzVPXAvailable = settingShowAllBalances; 
	bool showzVPXUnconfirmed = settingShowAllBalances;
	bool showzVPXImmature = settingShowAllBalances;
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("VPX")
    updateDisplayUnit();
}
...
// TFinch has edited it to work with Privix
...
void OverviewPage::updatBlockChainInfo()
{
    if (masternodeSync.IsBlockchainSynced()) {
        ...
        double BlockReward = GetBlockValue(chainActive.Height());
        double BlockRewardvpxcoin = static_cast<double>(BlockReward / COIN);
        ...
        ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardvpxcoin, 'f', 1));
        ui->label_Supply_value->setText(QString::number(chainActive.Tip()->nMoneySupply / COIN).append(" VPX"));
