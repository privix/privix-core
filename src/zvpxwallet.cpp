...
CzVPXWallet::CzVPXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zvpx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVPXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVPXSeed_deprecated()) {
                    LogPrintf("%s: Updated zVPX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zvpx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zvpx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzVPXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZVPXCount(nCountLastUsed);
    else if (!walletdb.ReadZVPXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVPXWallet::Lock()
{
    seedMaster = 0;
}

void CzVPXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVPXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVPX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVPXWallet::LoadMintPoolFromDB()
{
...
void CzVPXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVPXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVPXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zvpxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzVPXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVPX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zvpxTracker which also adds to database
    pwalletMain->zvpxTracker->Add(dMint, true);
...
void CzVPXWallet::SeedToZVPX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzVPXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVPXWallet::GenerateDeterministicZVPX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVPXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVPXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
