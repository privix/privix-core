...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 VPX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zVPX send
enum ZerocoinSpendStatus {
    ZVPX_SPEND_OKAY = 0,                            // No error
    ZVPX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZVPX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZVPX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZVPX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZVPX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZVPX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZVPX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZVPX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZVPX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZVPX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZVPX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZVPX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZVPX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZVPX_SPENT_USED_ZVPX = 14,                      // Coin has already been spend
    ZVPX_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZVPX_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZVpxBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZVPXOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzvpxAuto) const;
    ...
    */
    ...
    CzVPXWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzVPXTracker> zvpxTracker;
    ...
    void setZWallet(CzVPXWallet* zwallet)
    {
        zwalletMain = zwallet;
        zvpxTracker = std::unique_ptr<CzVPXTracker>(new CzVPXTracker(strWalletFile));
    }

    CzVPXWallet* getZWallet() { return zwalletMain; }
    ...
    void setZVpxAutoBackups(bool fEnabled)
    {
