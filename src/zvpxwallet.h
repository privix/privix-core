...
#ifndef PRIVIX_ZVPXWALLET_H
#define PRIVIX_ZVPXWALLET_H
...
public:
    CzVPXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZVPX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZVPX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //PRIVIX_ZVPXWALLET_H
