...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"privixprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"privixprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsHex(params[0].get_str())) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Privix address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
	if (fHelp || params.size() != 1)
		throw runtime_error(
			"dumpprivkey \"privixaddress\"\n"
			"\nReveals the private key corresponding to 'privixaddress'.\n"
			"Then the importprivkey can be used with this output\n"
			"\nArguments:\n"
			"1. \"privixaddress\"   (string, required) The privix address for the private key\n"
			"\nResult:\n"
...
UniValue dumpprivkey_vpx(const UniValue& params, bool fHelp)
{
	...
	if (params.size() == 1 || !AuthorizationHelper::inst().authorize(__FUNCTION__ + params[0].get_str(), params[1].get_str()))
	{
		std::string warning =
			...
			"Privix team members will never ask for this command's output and it is not needed for masternode setup or diagnosis!\n";
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "they will have FULL ACCESS to STEAL your Privix. Giving this file to someone\n"
            "is the same thing as giving them all of the Privix in your wallet! Never send\n"
            "this file to ANYONE that you do not trust with all of your Privix!!!\n"
            "\nArguments:\n"
...
/*    LOCK2(cs_main, pwalletMain->cs_wallet);
    ...
    // produce output
    file << strprintf("# Wallet dump created by Privix %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue dumpallprivatekeys(const UniValue& params, bool fHelp)
{
	...
	// produce output
	file << strprintf("# Wallet private key dump file created by Privix %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
	file << strprintf("# * Created on %s\n", EncodeDumpTime(GetTime()));
	file << strprintf("# * Best block at time of backup was %i (%s),\n", chainActive.Height(), chainActive.Tip()->GetBlockHash().ToString());
	file << strprintf("#   mined on %s\n", EncodeDumpTime(chainActive.Tip()->GetBlockTime()));
	file << "\n";
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"privixaddress\"\n"
            "\nEncrypts a private key corresponding to 'privixaddress'.\n"
            "\nArguments:\n"
            "1. \"privixaddress\"   (string, required) The privix address for the private key (you must hold the key already)\n"
