...
VERSION=
commit=false
url=https://github.com/privixproject/privix
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the privix, gitian-builder, gitian.sigs, and privix-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/privixproject/privix
...
then
	# Make output folder
	mkdir -p "./privix-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit privix=${COMMIT} --url privix=${url} ../privix/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../privix/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/privix-*.tar.gz build/out/src/privix-*.tar.gz ../privix-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit privix=${COMMIT} --url privix=${url} ../privix/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/privix-*.zip build/out/privix-*.exe ../privix-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit privix=${COMMIT} --url privix=${url} ../privix/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/privix-*.tar.gz build/out/privix-*.dmg ../privix-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit privix=${COMMIT} --url privix=${url} ../privix/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../privix/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/privix-*.tar.gz build/out/src/privix-*.tar.gz ../privix-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../privix/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/privix-*win64-setup.exe ../privix-binaries/${VERSION}
	    mv build/out/privix-*win32-setup.exe ../privix-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../privix/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/privix-osx-signed.dmg ../privix-binaries/${VERSION}/privix-${VERSION}-osx.dmg
	fi
	popd || exit
